...
const createVideoModelForm = (model?: Partial<AdminVideoModel>): VideoModelForm => ({
  ...
  maxReferenceVideos: Number(model?.maxReferenceVideos || 0),
  maxTotalReferences: Number(model?.maxTotalReferences || model?.maxReferenceImages || 1),
  referenceImageMode: model?.referenceImageMode || 'general',
  ...
  resolutionOptions: [],
  resolutionOptionsInput: arrayToInput(model?.resolutionOptions || ['720p']),
  ...
  const saveEditor = useCallback(async () => {
    ...
    try {
      ...
      if (editor.kind === 'video-model') {
        const payload: AdminVideoModelPayload = { ...videoModelForm, id: slugify(videoModelForm.id, 'video-model'), label: videoModelForm.label.trim(), description: videoModelForm.description?.trim() || '', modelFamily: videoModelForm.modelFamily.trim(), routeFamily: videoModelForm.routeFamily.trim(), requestModel: videoModelForm.requestModel?.trim() || '', referenceLabels: inputToArray(videoModelForm.referenceLabelsInput), aspectRatioOptions: inputToArray(videoModelForm.aspectRatioOptionsInput), resolutionOptions: inputToArray(videoModelForm.resolutionOptionsInput), durationOptions: inputToArray(videoModelForm.durationOptionsInput), defaultAspectRatio: String(videoModelForm.defaultAspectRatio || '').trim() || '16:9', defaultResolution: String(videoModelForm.defaultResolution || '').trim() || '720p', defaultDuration: String(videoModelForm.defaultDuration || '').trim() || '4', selectorCost: roundNonNegativePoint(videoModelForm.selectorCost || 0, 0), pricingMode: videoModelForm.pricingMode === 'per_second' ? 'per_second' : 'fixed', pointCostPerSecond: roundNonNegativePoint(videoModelForm.pointCostPerSecond || 0, 0), maxReferenceImages: Math.max(0, Number(videoModelForm.maxReferenceImages || 0)), maxReferenceVideos: Math.max(0, Number(videoModelForm.maxReferenceVideos || 0)), maxTotalReferences: Math.max(0, Number(videoModelForm.maxTotalReferences || 0)), referenceImageMode: videoModelForm.referenceImageMode || 'general', supportsVideoReference: videoModelForm.supportsVideoReference === true, promptMaxLength: videoModelForm.promptMaxLength === null || videoModelForm.promptMaxLength === undefined || String(videoModelForm.promptMaxLength).trim() === '' ? null : Math.max(0, Number(videoModelForm.promptMaxLength)), sortOrder: Number(videoModelForm.sortOrder || 0) };
